  ...
  *n_values = 0;
  switch (ckey)
    ...
    case G_SLICE_CONFIG_CONTENTION_COUNTER:
      ...
      *n_values = i;
      return (gint64*)g_memdup (array, sizeof (array[0]) * *n_values);
    default:
      return NULL;
    }
}
...
{
  ThreadMemory *tmem = (ThreadMemory*)g_private_get (&private_thread_memory);
  if (G_UNLIKELY (!tmem))
    {
      ...
      tmem = (ThreadMemory*)g_malloc0 (sizeof (ThreadMemory) + sizeof (Magazine) * 2 * n_magazines);
                              ...
                              gsize      count) /* must be >= MIN_MAGAZINE_SIZE */
{
  ...
  magazine_chain_count (current) = (ChunkLink*) count;
  /* stamp magazine */
  magazine_cache_update_stamp();
  magazine_chain_stamp (current) = (ChunkLink*) GUINT_TO_POINTER (allocator->last_stamp);
  ...
  if (!allocator->magazines[ix])
    {
      ...
      g_mutex_unlock (&allocator->magazine_mutex);
      g_mutex_lock (&allocator->slab_mutex);
      head = (ChunkLink*)slab_allocator_alloc_chunk (chunk_size);
      head->data = NULL;
      chunk = head;
      for (i = 1; i < magazine_threshold; i++)
        {
          chunk->next = (ChunkLink*)slab_allocator_alloc_chunk (chunk_size);
          chunk = chunk->next;
          chunk->data = NULL;
        }
      chunk->next = NULL;
...
{
  ThreadMemory *tmem = (ThreadMemory*)data;
                    ...
                    gsize      chunk_size)
{
  ...
  guint8 *mem = (guint8*)aligned_memory;
  guint i;
  if (!mem)
    {
  ...
  if (!compat_valloc_trash)
    {
      ...
      guint8 *mem = (guint8*)malloc (n_pages * sys_page_size);
      err = errno;
      if (mem)
        {
          gint i = n_pages;
                 ...
                 SmcVType value)
{
  ...
  if (!smc_tree_root)
    {
      smc_tree_root = (SmcBranch**)calloc (SMC_TRUNK_COUNT, sizeof (smc_tree_root[0]));
      if (!smc_tree_root)
        smc_tree_abort (errno);
    }
  if (!smc_tree_root[ix0])
    {
      smc_tree_root[ix0] = (SmcBranch*) calloc (SMC_BRANCH_COUNT, sizeof (smc_tree_root[0][0]));
      if (!smc_tree_root[ix0])
        smc_tree_abort (errno);
    }
