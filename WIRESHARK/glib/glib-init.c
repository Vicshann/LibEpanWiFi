...
#endif
GLogLevelFlags g_log_msg_prefix = GLogLevelFlags(G_LOG_LEVEL_ERROR | G_LOG_LEVEL_WARNING | G_LOG_LEVEL_CRITICAL | G_LOG_LEVEL_DEBUG);
...
g_messages_prefixed_init (void)
{
  ...
  g_log_msg_prefix = (GLogLevelFlags)g_parse_debug_envvar ("G_MESSAGES_PREFIXED", keys, G_N_ELEMENTS (keys), g_log_msg_prefix);
}
...
g_debug_init (void)
{
  ...
  flags = (GLogLevelFlags)g_parse_debug_envvar ("G_DEBUG", keys, G_N_ELEMENTS (keys), 0);

  *(int*)&g_log_always_fatal |= flags & G_LOG_LEVEL_MASK;
