...
{
  CRITICAL_SECTION *impl = (CRITICAL_SECTION*)mutex->p;
...
{
  g_rec_mutex_impl_free ((CRITICAL_SECTION*)mutex->p);
}
...
{
  LeaveCriticalSection ((CRITICAL_SECTION*)mutex->p);
}
      ...
      if (impl == 0)
        {
          ...
          if (key->notify != NULL)
            {
              destructor = (GPrivateDestructor*)malloc (sizeof (GPrivateDestructor));
               ...
               */
              if (InterlockedCompareExchangePointer ((volatile PVOID*)&g_private_destructors, destructor,
...
{
  GThreadWin32 *self = (GThreadWin32*)data;
...
g_thread_xp_waiter_get (void)
{
  ...
  waiter = (GThreadXpWaiter*)TlsGetValue (g_thread_xp_waiter_tls);

  if G_UNLIKELY (waiter == NULL)
    {
      waiter = (GThreadXpWaiter*)malloc (sizeof (GThreadXpWaiter));
      if (waiter == NULL)
  ...
  if G_UNLIKELY (result == NULL)
    {
      ...
      if (result == NULL)
        {
          result = (GThreadSRWLock*)malloc (sizeof (GThreadSRWLock));
...
{
  GThreadSRWLock *lock = g_thread_xp_get_srwlock ((GThreadSRWLock* volatile*)mutex);
...
g_thread_xp_get_condition_variable (GThreadXpCONDITION_VARIABLE * volatile *cond)
{
  ...
  if G_UNLIKELY (result == NULL)
    {
      result = (GThreadXpCONDITION_VARIABLE*)malloc (sizeof (GThreadXpCONDITION_VARIABLE));
      ...
      if (InterlockedCompareExchangePointer ((volatile PVOID*)cond, result, NULL) != NULL)
        {
          free (result);
          result = *cond;
        }
...
g_thread_xp_SleepConditionVariableSRW (gpointer cond,
                                       ...
                                       ULONG    flags)
{
  GThreadXpCONDITION_VARIABLE *cv = g_thread_xp_get_condition_variable ((GThreadXpCONDITION_VARIABLE*volatile*)cond);
...
#define GET_FUNC(name) if ((*(void**)&native_vtable.name = (void *) GetProcAddress (kernel32, #name)) == NULL) return FALSE
