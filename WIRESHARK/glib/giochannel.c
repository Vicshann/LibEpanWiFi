      ...
      if (flush)
        {
          ...
          g_io_channel_set_flags (channel, (GIOFlags)(flags & ~G_IO_FLAG_NONBLOCK), NULL);
  ...
  if (channel->encoding)
    {
      if (channel->encoded_read_buf && (channel->encoded_read_buf->len > 0))
        *(int*)condition |= G_IO_IN; /* Only return if we have full characters */
    }
  else
    {
      if (channel->read_buf && (channel->read_buf->len > 0))
        *(int*)condition |= G_IO_IN;
    }

  if (channel->write_buf && (channel->write_buf->len < channel->buf_size))
    *(int*)condition |= G_IO_OUT;
			    ...
			    gint         length)
{
  ...
  g_free (channel->line_term);
  channel->line_term = (gchar*)(line_term ? g_memdup (line_term, length) : NULL);
                        ...
                        GError     **error)
{
  ...
  return (*channel->funcs->io_set_flags) (channel,
					  (GIOFlags)(flags & G_IO_FLAG_SET_MASK),
					  error);
}
...
g_io_channel_get_flags (GIOChannel *channel)
{
  ...
  if (channel->is_seekable)
    *(int*)&flags |= G_IO_FLAG_IS_SEEKABLE;
  if (channel->is_readable)
    *(int*)flags |= G_IO_FLAG_IS_READABLE;
  if (channel->is_writeable)
    *(int*)flags |= G_IO_FLAG_IS_WRITABLE;
