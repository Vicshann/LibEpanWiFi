      ...
      while (*aliases != '\0')
        {
          ...
          alias_array = (const char**)g_hash_table_lookup (alias_hash, canonical);
          if (alias_array)
            {
...
_g_charset_get_aliases (const char *canonical_name)
{
  ...
  return (const char**)g_hash_table_lookup (alias_hash, canonical_name);
}
...
{
  GCharsetCache *cache = (GCharsetCache*)data;
  g_free (cache->raw);
...
g_get_charset (const char **charset)
{
  ...
  GCharsetCache *cache = (GCharsetCache*)g_private_get (&cache_private);
  const gchar *raw;
...
{
  GLanguageNamesCache *cache = (GLanguageNamesCache*)data;
...
g_get_language_names (void)
{
  ...
  GLanguageNamesCache *cache = (GLanguageNamesCache*)g_private_get (&cache_private);
  const gchar *value;
