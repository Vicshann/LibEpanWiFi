...
g_main_context_unref (GMainContext *context)
{
  ...
  for (i = 0; i < context->pending_dispatches->len; i++)
    g_source_unref_internal ((GSource*)context->pending_dispatches->pdata[i], context, FALSE);
...
g_main_context_push_thread_default (GMainContext *context)
{
  ...
  stack = (GQueue*)g_private_get (&thread_context_stack);
  if (!stack)
    {
      stack = g_queue_new ();
...
g_main_context_get_thread_default (void)
{
  ...
  stack = (GQueue*)g_private_get (&thread_context_stack);
  if (stack)
    return (GMainContext*)g_queue_peek_head (stack);
  else
    return NULL;
}
...
g_source_iter_next (GSourceIter *iter, GSource **source)
{
  ...
  if (!next_source)
	...
	iter->current_list = iter->context->source_lists;
      ...
      if (iter->current_list)
	{
	  GSourceList *source_list = (GSourceList*)iter->current_list->data;
			       ...
			       gboolean      create)
{
  ...
  for (iter = context->source_lists; iter != NULL; last = iter, iter = iter->next)
    {
      source_list = (GSourceList*)iter->data;
                          ...
                          gboolean      do_wakeup)
{
  ...
  if (!SOURCE_BLOCKED (source))
    {
      ...
      while (tmp_list)
        {
          g_main_context_add_poll_unlocked (context, source->priority, (GPollFD*)tmp_list->data);
  ...
  while (tmp_list)
    {
      g_source_attach_unlocked ((GSource*)tmp_list->data, context, FALSE);
      tmp_list = tmp_list->next;
    }
			   ...
			   gboolean      have_lock)
{
  ...
  if (!SOURCE_DESTROYED (source))
      ...
      if (!SOURCE_BLOCKED (source))
	{
	  ...
	  while (tmp_list)
	    {
	      g_main_context_remove_poll_unlocked (context, (GPollFD*)tmp_list->data);
	      tmp_list = tmp_list->next;
	    }
      ...
      while (source->priv->child_sources)
        g_child_source_remove_internal ((GSource*)source->priv->child_sources->data, context);
...
{
  GSourceCallback *callback = (GSourceCallback*)cb_data;
  ...
  if (context)
      ...
      if (!SOURCE_BLOCKED (source))
	{
	  ...
	  while (tmp_list)
	    {
	      g_main_context_remove_poll_unlocked (context, (GPollFD*)tmp_list->data);
	      g_main_context_add_poll_unlocked (context, priority, (GPollFD*)tmp_list->data);
  ...
  if (source->priv->child_sources)
    {
      ...
      while (tmp_list)
	{
	  g_source_set_priority_unlocked ((GSource*)tmp_list->data, context, priority);
	  tmp_list = tmp_list->next;
	}
    }
}
			 ...
			 gboolean      have_lock)
{
  ...
  if (source->ref_count == 0)
      ...
      if (source->source_funcs->finalize)
	{
      ...
      while (source->priv->child_sources)
        {
          GSource *child_source = (GSource*)source->priv->child_sources->data;
                                  ...
                                  guint         source_id)
{
  ...
  LOCK_CONTEXT (context);
  source = (GSource*)g_hash_table_lookup (context->sources, GUINT_TO_POINTER (source_id));
  UNLOCK_CONTEXT (context);
...
g_clock_win32_init (void)
{
  ...
  if (kernel32 != NULL)
    *(void**)&g_GetTickCount64 = (void *) GetProcAddress (kernel32, "GetTickCount64");
...
get_dispatch (void)
{
  ...
  dispatch = (GMainDispatch*)g_private_get (&depth_private);
  ...
  if (source->context)
    {
      ...
      while (tmp_list)
        {
          g_main_context_remove_poll_unlocked (source->context, (GPollFD*)tmp_list->data);
          tmp_list = tmp_list->next;
        }
  ...
  if (source->priv && source->priv->child_sources)
    {
      ...
      while (tmp_list)
	{
	  block_source ((GSource*)tmp_list->data);
	  tmp_list = tmp_list->next;
	}
    }
}
...
unblock_source (GSource *source)
{
  ...
  while (tmp_list)
    {
      g_main_context_add_poll_unlocked (source->context, source->priority, (GPollFD*)tmp_list->data);
      tmp_list = tmp_list->next;
    }
  ...
  if (source->priv && source->priv->child_sources)
    {
      ...
      while (tmp_list)
	{
	  unblock_source ((GSource*)tmp_list->data);
	  tmp_list = tmp_list->next;
	}
    }
}
...
g_main_dispatch (GMainContext *context)
{
  ...
  for (i = 0; i < context->pending_dispatches->len; i++)
    {
      GSource *source = (GSource*)context->pending_dispatches->pdata[i];
  ...
  if (context->owner_count == 0)
    {
      ...
      if (context->waiters)
	{
	  GMainWaiter *waiter = (GMainWaiter*)context->waiters->data;
          ...
          if (result == FALSE)
            {
              ...
              for (tmp_list = source->priv->fds; tmp_list; tmp_list = tmp_list->next)
                {
                  GPollFD *pollfd = (GPollFD*)tmp_list->data;
