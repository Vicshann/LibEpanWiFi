...
compare_mapping_entry (gconstpointer user_data,
                       gconstpointer data)
{
  const struct mapping_entry *entry = (mapping_entry*)data;
  const gunichar *key = (gunichar*)user_data;
  gunichar src_0;
                   ...
                   gint                       *key_consumed)
{
  ...
  hit = (mapping_entry*)bsearch (key, mapping, mapping_size, sizeof (struct mapping_entry), compare_mapping_entry);
  ...
  *result_len = get_length (hit->ascii);
  ...
  return (gchar*)get_ascii_item(ascii_table, hit->ascii);
}
...
compare_locale_entry (gconstpointer user_data,
                      gconstpointer data)
{
  const struct locale_entry *entry = (locale_entry*)data;
  const gchar *key = (gchar*)user_data;
                               ...
                               guint       *item_id)
{
  ...
  hit = (locale_entry*)bsearch (key, locale_index, G_N_ELEMENTS (locale_index), sizeof (struct locale_entry), compare_locale_entry);
