             ...
             ...)
{
  ...
  while ((option = va_arg (args, char *)))
    {
      if (g_strcmp0 ((char*)option, "no_g_set_prgname") == 0)
                    ...
                    GTestFixtureFunc  data_teardown)
{
  ...
  *(void**)&tc->fixture_setup = (void*) data_setup;
  *(void**)&tc->fixture_test = (void*) data_test;
  *(void**)&tc->fixture_teardown = (void*) data_teardown;
...
{
  const GTestSuite *suite = (GTestSuite*)l;
  const gchar *str = (gchar*)s;
  ...
  if (slash)
    {
      for (iter = suite->suites; iter; iter = iter->next)
        {
          GTestSuite *child_suite = (GTestSuite*)iter->data;
  ...
  if (strstr (test_run_name, "/subprocess"))
    {
      ...
      for (iter = test_paths; iter; iter = iter->next)
        {
          if (!strcmp (test_run_name, (char*)iter->data))
            {
              found = TRUE;
              break;
            }
                           ...
                           const char *path)
{
  ...
  for (slist = reversed; slist; slist = slist->next)
    {
      GTestCase *tc = (GTestCase*)slist->data;
  ...
  for (slist = reversed; slist; slist = slist->next)
    {
      GTestSuite *ts = (GTestSuite*)slist->data;
...
g_test_run_suite (GTestSuite *suite)
{
  ...
  while (my_test_paths)
    {
      const char *rest, *path = (char*)my_test_paths->data;
                ...
                guint64 timeout)
{
  ...
  source = g_io_create_watch (data.stdout_io, (GIOCondition)(G_IO_IN | G_IO_ERR | G_IO_HUP));
  ...
  source = g_io_create_watch (data.stderr_io, (GIOCondition)(G_IO_IN | G_IO_ERR | G_IO_HUP));
  g_source_set_callback (source, (GSourceFunc) child_read, &data, NULL);
  g_source_attach (source, context);
  g_source_unref (source);
...
g_test_log_extract (GTestLogBuffer *tbuffer)
{
  ...
  if (tbuffer->data->len < mlength)
    return FALSE;
  msg.log_type = (GTestLogType)net_int (&p);
