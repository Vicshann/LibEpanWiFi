...
g_getenv (const gchar *variable)
{
  ...
  wname = (wchar_t*)g_utf8_to_utf16 (variable, -1, NULL, NULL, NULL);
  ...
  value = g_utf16_to_utf8 ((gunichar2*)wvalue, -1, NULL, NULL, NULL);
          ...
          gboolean     overwrite)
{
  ...
  wname = (wchar_t*)g_utf8_to_utf16 (variable, -1, NULL, NULL, NULL);
  wvalue = (wchar_t*)g_utf8_to_utf16 (value, -1, NULL, NULL, NULL);
  tem = g_strconcat (variable, "=", value, NULL);
  wassignment = (wchar_t*)g_utf8_to_utf16 (tem, -1, NULL, NULL, NULL);
...
g_listenv (void)
{
  ...
  while (*q)
    {
      result[j] = g_utf16_to_utf8 ((gunichar2*)q, -1, NULL, NULL, NULL);
      if (result[j] != NULL)
        {
          eq = strchr (result[j], '=');
          if (eq && eq > result[j])
...
g_get_environ (void)
{
  ...
  strings = (gunichar2*)GetEnvironmentStringsW ();
  for (n = 0, i = 0; strings[n]; i++)
    n += wcslen ((wchar_t*)strings + n) + 1;
  ...
  FreeEnvironmentStringsW ((wchar_t*)strings);
  result[i] = NULL;
