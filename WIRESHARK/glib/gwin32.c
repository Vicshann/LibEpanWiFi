...
g_win32_error_message (gint error)
{
  ...
  if (msg != NULL)
	...
	msg[nchars-2] = '\0';
      ...
      retval = g_utf16_to_utf8 ((gunichar2*)msg, -1, NULL, NULL, NULL);
...
g_win32_get_package_installation_directory_of_module (gpointer hmodule)
{
  ...
  if (!GetModuleFileNameW ((HMODULE)hmodule, wc_fn, MAX_PATH))
    return NULL;

  filename = g_utf16_to_utf8 ((gunichar2*)wc_fn, -1, NULL, NULL, NULL);
...
get_package_directory_from_module (const gchar *module_name)
{
  ...
  fn = (gchar*)g_hash_table_lookup (module_dirs, module_name ? module_name : "");
  ...
  if (module_name)
    {
      wchar_t *wc_module_name = (wchar_t*)g_utf8_to_utf16 (module_name, -1, NULL, NULL, NULL);
  ...
  if (retval == NULL)
    {
       ...
       */
      wchar_t *wname = (wchar_t*)g_utf8_to_utf16 (utf8filename, -1, NULL, NULL, NULL);
      if (wname != NULL)
	{
	  wchar_t wshortname[MAX_PATH + 1];
	  if (GetShortPathNameW (wname, wshortname, G_N_ELEMENTS (wshortname)))
	    {
	      gchar *tem = g_utf16_to_utf8 ((gunichar2*)wshortname, -1, NULL, NULL, NULL);
...
g_win32_get_command_line (void)
{
  ...
  for (i = 0; i < n; i++)
    result[i] = g_utf16_to_utf8 ((gunichar2*)args[i], -1, NULL, NULL, NULL);
  result[i] = NULL;
