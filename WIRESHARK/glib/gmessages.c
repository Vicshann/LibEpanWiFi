...
g_log_domain_new_L (const gchar *log_domain)
{
  ...
  domain->fatal_mask = (GLogLevelFlags)G_LOG_FATAL_MASK;
...
g_log_set_always_fatal (GLogLevelFlags fatal_mask)
{
   ...
   */
  *(int*)&fatal_mask &= (1 << G_LOG_LEVEL_USER_SHIFT) - 1;
  /* force errors to be fatal */
  *(int*)&fatal_mask |= G_LOG_LEVEL_ERROR;
  /* remove bogus flag */
  *(int*)&fatal_mask &= ~G_LOG_FLAG_FATAL;
	...
	va_list	       args)
{
  ...
  *(int*)&log_level &= G_LOG_LEVEL_MASK;
  if (!log_level)
    return;
  ...
  if (expected_messages)
    {
      GTestExpectedMessage *expected = (GTestExpectedMessage*)expected_messages->data;
      ...
      if (g_strcmp0 (expected->log_domain, log_domain) == 0 &&
      ...
      else if ((log_level & G_LOG_LEVEL_DEBUG) != G_LOG_LEVEL_DEBUG)
        {
          ...
          *(int*)&log_level |= G_LOG_FLAG_FATAL;
        }
    }
  ...
  for (i = g_bit_nth_msf (log_level, -1); i >= 0; i = g_bit_nth_msf (log_level, i))
      ...
      if (log_level & test_level)
	{
	  ...
	  if (was_fatal)
	    *(int*)&test_level |= G_LOG_FLAG_FATAL;
	  if (was_recursion)
	    *(int*)&test_level |= G_LOG_FLAG_RECURSION;
	  ...
	  if (depth)
	    *(int*)&test_level |= G_LOG_FLAG_RECURSION;
	  depth++;
	  *(int*)&domain_fatal_mask = domain ? domain->fatal_mask : G_LOG_FATAL_MASK;
  ...
  if (expected_messages)
    {
      ...
      expected = (GTestExpectedMessage*)expected_messages->data;
...
{
  gchar c;
  return _g_vsnprintf (0, 0, format, args) + 1;   // _g_vsnprintf (&c, 1, format, args) 
}
