    ...
    struct wtap_pkthdr *phdr, const guint8 *buf, gboolean add_to_packet_list)
{
  ...
  if (dfcode != NULL) {
    ...
    if (fdata->flags.passed_dfilter) {
       ...
       * as depended upon.
       */
    ////  g_slist_foreach(edt->pi.dependent_frames, find_and_mark_frame_depended_upon, cf->frames);   // Vicshann
    }
  } else
...
#endif
  /* and save the packet */
  if (/*!wtap_dump(args->pdh, &hdr, pd, &err, &err_info)*/0) {
    if (err < 0) {
      /* Wiretap error. */
      switch (err) {
...
cf_can_write_with_wiretap(capture_file *cf)
{
  /* We don't care whether we support the comments in this file or not;
     ...
     comments. */
  return 0; // wtap_dump_can_write(cf->linktypes, 0);
}
