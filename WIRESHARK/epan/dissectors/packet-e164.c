...
dissect_e164_msisdn(tvbuff_t *tvb, proto_tree *tree, int offset, int length, e164_encoding_t encoding)
{
	...
	switch (encoding) {
	case E164_ENC_UTF8:
		msisdn_str = (gchar*)tvb_get_string_enc(wmem_packet_scope(), tvb, offset, length, ENC_UTF_8);
		break;
	case E164_ENC_BCD:
		msisdn_str = (gchar*)tvb_bcd_dig_to_wmem_packet_str(tvb, offset, length, NULL, FALSE);
		break;
	case E164_ENC_BINARY:
	default:
