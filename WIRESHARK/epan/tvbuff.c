...
tvb_get_ucs_4_string(wmem_allocator_t *scope, tvbuff_t *tvb, const gint offset, gint length, const guint encoding)
{
	...
	ptr = ensure_contiguous(tvb, offset, length);
	return (gchar*)get_ucs_4_string(scope, ptr, length, encoding);
}
	...
	const gint bit_offset, gint no_of_chars)
{
	...
	return (gchar*)get_ts_23_038_7bits_string(scope, ptr, bit_offset, no_of_chars);
}
	...
	const gint bit_offset, gint no_of_chars)
{
	...
	ptr = ensure_contiguous(tvb, in_offset, length);
	return (gchar*)get_ascii_7bits_string(scope, ptr, bit_offset, no_of_chars);
}
			     ...
			     const gint length, const guint encoding)
{
	...
	case ENC_UCS_4:
		strptr = (guint8*)tvb_get_ucs_4_string(scope, tvb, offset, length,
		    encoding & ENC_LITTLE_ENDIAN);
		break;
	...
	case ENC_3GPP_TS_23_038_7BITS:
		{
			...
			strptr = (guint8*)tvb_get_ts_23_038_7bits_string(scope, tvb, bit_offset, no_of_chars);
		}
		break;
...
tvb_get_ucs_2_stringz(wmem_allocator_t *scope, tvbuff_t *tvb, const gint offset, gint *lengthp, const guint encoding)
{
	...
	if (lengthp)
		*lengthp = size;
	return (gchar*)get_ucs_2_string(scope, ptr, size, encoding);
}
...
tvb_get_utf_16_stringz(wmem_allocator_t *scope, tvbuff_t *tvb, const gint offset, gint *lengthp, const guint encoding)
{
	...
	if (lengthp)
		*lengthp = size;
	return (gchar*)get_utf_16_string(scope, ptr, size, encoding);
}
...
tvb_get_ucs_4_stringz(wmem_allocator_t *scope, tvbuff_t *tvb, const gint offset, gint *lengthp, const guint encoding)
{
	...
	if (lengthp)
		*lengthp = size;
	return (gchar*)get_ucs_4_string(scope, ptr, size, encoding);
}
...
tvb_get_stringz_enc(wmem_allocator_t *scope, tvbuff_t *tvb, const gint offset, gint *lengthp, const guint encoding)
{
	...
	case ENC_UTF_16:
		strptr = (guint8*)tvb_get_utf_16_stringz(scope, tvb, offset, lengthp,
		    encoding & ENC_LITTLE_ENDIAN);
		break;

	case ENC_UCS_2:
		strptr = (guint8*)tvb_get_ucs_2_stringz(scope, tvb, offset, lengthp,
		    encoding & ENC_LITTLE_ENDIAN);
		break;

	case ENC_UCS_4:
		strptr = (guint8*)tvb_get_ucs_4_stringz(scope, tvb, offset, lengthp,
		    encoding & ENC_LITTLE_ENDIAN);
		break;
...
_tvb_get_nstringz(tvbuff_t *tvb, const gint offset, const guint bufsize, guint8* buffer, gint *bytes_copied)
{
	...
	check_offset_length(tvb, offset, -1, &abs_offset, (guint*)&len);
