...
{
      wchar_t *woldfilename = (wchar_t*)g_utf8_to_utf16 (oldfilename, -1, NULL, NULL, NULL);
      ...
      wnewfilename = (wchar_t*)g_utf8_to_utf16 (newfilename, -1, NULL, NULL, NULL);
        ...
        ws_statb64 *buf)
{
      ...
      while (len > 0 && G_IS_DIR_SEPARATOR (wfilename[len-1]))
        len--;
      if (len > 0 &&
          (!g_path_is_absolute (filename) || len > (size_t) (g_path_skip_root ((gchar*)filename) - filename)))
        wfilename[len] = '\0';
         ...
         const gchar *mode)
{
      ...
      wmode = (wchar_t*)g_utf8_to_utf16 (mode, -1, NULL, NULL, NULL);
...
init_dll_load_paths()
{
      ...
      if (GetModuleFileNameW(NULL, path_w, MAX_PATH) == 0 || GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
            return FALSE;
      }

      if (!program_path) {
            gchar *app_path;
            app_path = g_utf16_to_utf8((gunichar2*)&path_w, -1, NULL, NULL, NULL);
            /* We could use PathRemoveFileSpec here but we'd have to link to Shlwapi.dll */
            program_path = g_path_get_dirname(app_path);
            g_free(app_path);
      }

      if (GetSystemDirectoryW(path_w, MAX_PATH) == 0) {
            return FALSE;
      }

      if (!system_path) {
            system_path = g_utf16_to_utf8((gunichar2*)&path_w, -1, NULL, NULL, NULL);
      }
...
ws_init_dll_search_path()
{
      ...
      if (!dll_dir_set && init_dll_load_paths()) {
            program_path_w = (wchar_t*)g_utf8_to_utf16(program_path, -1, NULL, NULL, NULL);
            SetCurrentDirectoryW(program_path_w);
...
ws_load_library(gchar *library_name)
{
      ...
      full_path_w = (wchar_t*)g_utf8_to_utf16(full_path, -1, NULL, NULL, NULL);
...
getenv_utf8(const char *varname)
{
        ...
        varnamew = (wchar_t*)g_utf8_to_utf16(varname, -1, NULL, NULL, NULL);
        if (varnamew == NULL) {
                return envvar;
        }
        ...
        envvar = g_utf16_to_utf8((gunichar2*)envvarw, -1, NULL, NULL, NULL);
        /* XXX - memleak */
