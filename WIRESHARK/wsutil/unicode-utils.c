...
utf_8to16(const char *utf8str)
{
  ...
  if (utf16buf[idx] == NULL) {
    ...
    utf16buf[idx] = (wchar_t*)g_malloc(utf16buf_len[idx] * sizeof(wchar_t));
  }
  ...
  while (MultiByteToWideChar(CP_UTF8, 0, utf8str,
     ...
     */
    utf16buf_len[idx] *= 2;
    utf16buf[idx] = (wchar_t*)g_realloc(utf16buf[idx], utf16buf_len[idx] * sizeof(wchar_t));
  }
...
utf_16to8(const wchar_t *utf16str)
{
  ...
  if (utf8buf[idx] == NULL) {
    ...
    utf8buf[idx] = (gchar*)g_malloc(utf8buf_len[idx]);
  }
  ...
  while (WideCharToMultiByte(CP_UTF8, 0, utf16str, -1,
     ...
     */
    utf8buf_len[idx] *= 2;
    utf8buf[idx] = (gchar*)g_realloc(utf8buf[idx], utf8buf_len[idx]);
  }
...
arg_list_utf_16to8(int argc, char *argv[]) {
  ...
  if (wc_argv && wc_argc == argc) {
    for (i = 0; i < argc; i++) {
      argv[i] = g_utf16_to_utf8((gunichar2*)wc_argv[i], -1, NULL, NULL, NULL);
    }
