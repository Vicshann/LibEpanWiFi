...
#ifndef HAVE_MKSTEMP
/* Generate a unique temporary file name from FTemplate.
   The last six characters of FTemplate must be TMP_FILE_SUFFIX;
   they are replaced with a string that makes the filename unique.
   Returns a file descriptor open on the file for reading and writing.  */
static int
mkstemp (char *FTemplate)
{
  ...
  len = strlen (FTemplate);
  if (len < 6 || strcmp (&FTemplate[len - 6], TMP_FILE_SUFFIX))
    {
      __set_errno (EINVAL);
      return -1;
    }

  if (g_snprintf (&FTemplate[len - 5], 6, "%.5u",
  ...
  for (i = 0; i < sizeof (letters); ++i)
    {
      ...
      FTemplate[len - 6] = letters[i];

      fd = ws_open (FTemplate, O_RDWR|O_BINARY|O_CREAT|O_EXCL, 0600);
      if (fd >= 0)
        return fd;
    }
...
#ifndef HAVE_MKDTEMP
/* Generate a unique temporary directory name from FTemplate.
   The last six characters of FTemplate must be TMP_FILE_SUFFIX;
   they are replaced with a string that makes the filename unique.
   Returns 0 on success or -1 on error (from mkdir(2)).  */
char *
mkdtemp (char *FTemplate)
{
  static const char letters[]
    = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
  size_t len;
