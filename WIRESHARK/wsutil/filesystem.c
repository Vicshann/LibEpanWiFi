...
find_last_pathname_separator(const char *path)
{
     ...
     */
    separator = (char*)strchr(path, '\0');     /* points to ending '\0' */
...
{
#ifdef _WIN32
    wchar_t prog_pathname_w[_MAX_PATH+2];
    char *prog_pathname;
    DWORD error;
    wchar_t *msg_w;
    guchar *msg;
     ...
     * program.
     */
    if (GetModuleFileNameW(NULL, prog_pathname_w, G_N_ELEMENTS(prog_pathname_w)) != 0 && GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
        /*
...
get_persdatafile_dir(void)
{
    ...
    if (u3devicedocumentpath != NULL) {
        ...
        if (SHGetSpecialFolderPath(NULL, tszPath, CSIDL_PERSONAL, FALSE)) {
            persdatafile_dir = g_utf16_to_utf8((gunichar2*)tszPath, -1, NULL, NULL, NULL);
            return persdatafile_dir;
        } else {
            return "";
        }
