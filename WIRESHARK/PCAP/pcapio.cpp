...
libpcap_write_file_header(FILE* pfile, int linktype, int snaplen, gboolean ts_nsecs, guint64 *bytes_written, int *err)
{
		...
		file_hdr.sigfigs = 0;   /* unknown, but also apparently unused */   /* accuracy of timestamps */
		file_hdr.snaplen = snaplen;   /* max length of captured packets, in octets */
        file_hdr.network = linktype;  /* data link type */
                     ...
                     guint64 *bytes_written, int *err)
{
		...
		rec_hdr.ts_sec = (guint32)sec; /* Y2.038K issue in pcap format.... */   /* timestamp seconds */
		rec_hdr.ts_usec = usec;                                                 /* timestamp microseconds (nsecs for PCAP_NSEC_MAGIC) */
		rec_hdr.incl_len = caplen;                                              /* number of octets of packet saved in file */
        rec_hdr.orig_len = len;                                                 /* actual length of packet */
